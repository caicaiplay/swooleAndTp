...
class Sw
{
    ...
    {
        // 实例化一个http服务器
        $this->http = new Swoole\WebSocket\Server(self::HOST, self::PORT);
        // 服务器配置
    ...
    public function onOpen($ws, $request)
    {
        ...
        if (!($redis->sAdd('request_id', $request->fd))) {
            return false;
        }
    }
...
//            echo json_encode($e->getMessage());
    ...
    public  function writeLog() {
        $arr = array_merge(['date' => time()],$_SERVER, $_POST, $_GET, $_FILES);
...
//        echo $logs;
        $path = __DIR__.'/log/'.date('Ym').'/'.date('d').'_cli.log';
//        var_dump($path);

        swoole_async_writefile($path, $logs, function($filename) {}, FILE_APPEND);
    }
    ...
    public function onTask($serv, $task_id, $from_id, $data) {
        ...
        $res = $taskClass->$method($data['data'], $serv);
    }
    // onfinish
    ...
    {
        Redis::getInstance()->sRem('request_id', $fd);
        echo "clientid:{$fd}\n";
    }
}
